...
def generate_axis(limit, k):
    ...
    # Generate powers of 2 up to K
    while 2**i < k:
        sequence.append(2**i)
        i += 1

    # Continue with increments of K
    value = k
    while value <= limit:
...
def get_last_math(output):

    if "\\boxed" in output:
        last_box = last_boxed_only_string(output)

        try:
            final = remove_boxed(last_box)
            final = normalize_final_answer(final)
        except Exception as e:
            # print("no number in box")
            final = get_last_number(output)
