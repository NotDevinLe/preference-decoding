...
if __name__ == "__main__":
    ...
    parser.add_argument("--p_path", type=str, default="../../results/mle/user1_by_training_size_p.jsonl")
    ...
    with open(args.p_path, "r") as f:
        for line in f:
            entry = json.loads(line)
            if entry["num_data_points"] == args.training_size:
                p = torch.tensor(entry["p_vector"], device=device, dtype=torch.float32)
    ...
    for n in n_values:
        ...
        with open("../../results/mle_bon.jsonl", "a") as f:
            f.write(json.dumps(result) + "\n")

    print(f"\n✅ Results saved to ../../results/mle_bon.jsonl")
