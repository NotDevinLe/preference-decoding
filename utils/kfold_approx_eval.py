...
parser.add_argument("--sample_size", type=int, required=True, help="Number of samples you use")
...
print(data['user'])
data = data['data']
data = random.sample(data, k=args.sample_size)
...
for train_ind, test_ind in folds.split(data):
    ...
    p = [p[i] for i in temp]
    fold_acc = get_approximation_accuracy(test, model_ds, p, base_prompt, attribute_prompts, device, tokenizer)
    print(f"Fold Accuracy: {fold_acc}")
    acc += fold_acc
acc /= 5

result = {"user": args.name, "n": args.sample_size, "accuracy": acc}
